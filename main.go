...
func main() {
    ...
    rp := newMultipleHostReverseProxy(backends...)
    ...
    if err := http.ListenAndServe(addr, rp); err != nil {
        log.Fatalln(err)
    }
}
...
func newMultipleHostReverseProxy(backends ...backend) *httputil.ReverseProxy {
    return &httputil.ReverseProxy{Director: weightedDirector(backends...), ErrorLog: logger}
}
